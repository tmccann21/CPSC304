...
const gamesPlayedRoutes: RouteRegistrar = (app: express.Application, db) => {
  ...
  //app.get('/gp/:time/:location/:team1Name/:team2Name', async (req: any, res) => {
    app.get('/gp/:team1Name/:team2Name/:location', async(req: any, res) => {
    try {
      ...
      const response = await controller.getGamesPlayed(req.params.team1Name, req.params.team2Name, req.params.location);
      res.json(response);
    } catch (err) {
      log.error(err.message || err);
      res.statusCode = 404;
